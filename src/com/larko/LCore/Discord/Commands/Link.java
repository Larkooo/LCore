...
public class Link implements CommandListener {
    ...
    public void onCommand(Member member, TextChannel textChannel, Message message, String[] args) {
        ...
        Iterator<JSONObject> mainCachedPlayersIterator = Main.cachedPlayersData.iterator();
        while(mainCachedPlayersIterator.hasNext()) {
            JSONObject player = mainCachedPlayersIterator.next();

            OfflinePlayer offlinePlayer = Bukkit.getOfflinePlayer(UUID.fromString(player.get("uuid").toString()));
            if (offlinePlayer == null) continue;
            if (!offlinePlayer.getName().equalsIgnoreCase(minecraftUsername)) continue;
            User user = member.getUser();
            if (player.containsKey("discordId")) {
                if (player.get("discordId").toString().equals(user.getId())) {
                    message.reply("This account is already linked to your discord account").queue();
                } else {
                    message.reply("This account is already linked to another discord account");
                }
                return;
            }
            ...
            TimerTask task = new TimerTask(){
                public void run(){
                    ...
                    if (BCrypt.checkpw(latestMessage.getContentRaw(), (String) ((JSONObject) player).get("password"))) {
                        ...
                        latestMessage.reply("Bad password. You can retry linking your minecraft LCore account by running the command again.");
                    }
